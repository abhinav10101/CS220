...
module Computer(
    ...
    input         clk,
    input         done_storing,
    input         copied_io_regs,     // <-- Added as an INPUT
    output reg    done,
    ...
    output        io_stall,
    output [2:0]  io_reg_index_out    // <-- Changed to 3 bits to count to 4 safely
);
    wire [7:0]  pc;
    ...
    Processor4 proc(
        ...
        .io_reg3          (out_reg3),
        .io_reg4          (out_reg4),
        .copied_io_regs   (copied_io_regs), // <-- Now driven by the input port
