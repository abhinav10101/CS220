...
module ALU7 (
    ...
    always @(*) begin
        ...
        case (opcode)
            ...
            `OP_LUI: begin
                // LUI shifts the 16-bit immediate to the upper 16 bits.
                // The immediate is natively preserved in the lower 16 bits of branch_offset.
                result = {branch_offset[15:0], 16'd0}; 
                result_valid = 1'b1;
            end

            //Loads 
            `OP_LB, `OP_LH, `OP_LW, `OP_LBU, `OP_LHU: begin
                result = src1 + branch_offset; // Base + offset
                result_valid = 1'b1;           // Loads write to register file
            end

            //  Stores 
            `OP_SB, `OP_SH, `OP_SW: begin
                result = src1 + branch_offset; // Base + offset
                result_valid = 1'b0;           // Stores do not write to register file
            end
